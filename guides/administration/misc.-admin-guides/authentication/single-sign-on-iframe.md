...
If you already have the Rocket.Chat inside an iframe you can use call commands inside the Rocket.Chat's an iframe, [there is a command to login the user passing a token](https://github.com/jpreynat/docs-3/blob/master/guides/administration/misc.-admin-guides/authentication/broken-reference/README.md), for that you need to make your own way to retrieve the token from your own APIs and then pass it to the iframe.
...
3. Your API will then call the [Rocket.Chat APIs](https://github.com/jpreynat/docs-3/blob/master/guides/administration/misc.-admin-guides/authentication/broken-reference/README.md) to check if the user exists
...
6. In your client, with the token in hands, call the [iframe methods to execute the login](https://github.com/jpreynat/docs-3/blob/master/guides/administration/misc.-admin-guides/authentication/broken-reference/README.md)
...
The other setting you configure here is the [API URL and Method ](https://github.com/jpreynat/docs-3/blob/master/guides/administration/misc.-admin-guides/authentication/broken-reference/README.md)to be called once a not authenticated user opens the Rocket.Chat. Then Rocket.Chat will make an HTTP call to you endpoint expecting a [JSON response containing the authToken](https://github.com/jpreynat/docs-3/blob/master/guides/administration/misc.-admin-guides/authentication/broken-reference/README.md), so you should use cookies to identify if the user calling your own API is authenticated and make the process to generate the Rocket.Chat's token and return it:
...
3. The API will then call the Rocket.Chat APIs to check if the user exists
   1. If it doesn't exists call the Rocket.Chat APIs to create the user
4. If the user exists, call the Rocket.Chat APIs to make the login and receive the token
5. Return the token in your API response in JSON format as [described here](https://github.com/jpreynat/docs-3/blob/master/guides/administration/misc.-admin-guides/authentication/broken-reference/README.md)
